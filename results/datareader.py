...
class YcbineoatReader:
  ...
  def get_color(self,i):
    ...
    return color
#
# Changes made by me to accomodate the multiple instances of objects start here
#
  def get_mask(self,i, mask_dir = None):
    if mask_dir is None:
      mask = cv2.imread(self.color_files[i].replace('rgb','masks'),-1)
    else:
      mask = cv2.imread(self.color_files[i].replace('rgb',f'masks/{mask_dir}'),-1)
    if len(mask.shape)==3:
      for c in range(3):
        if mask[...,c].sum()>0:
          mask = mask[...,c]
    ...
    return mask
#
# Changes made by me to accomodate the multiple instances of objects end here
#
